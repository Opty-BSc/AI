...
import time
from ScotlandYard import SearchProblem

flag = False

with open("Resources/coords.pickle", "rb") as fp:  # Unpickling
    coords = pickle.load(fp)

with open("Resources/mapasgraph.pickle", "rb") as fp:  # Unpickling
    AA = pickle.load(fp)
U = AA[1]
...
def plotpath(P, coords):
    ...
    img = plt.imread('Resources/maps.png')
    plt.imshow(img)
    colors = ['r.-', 'g+-', 'b^-']
    I = P[0][1]
    for agind in range(len(P[0][1])):
        ...
        for tt in P:
            nst = tt[1][agind] - 1
            plt.plot([coords[st][0], coords[nst][0]], [coords[st][1], coords[nst][1]],
                     colors[agind])
            st = nst
    plt.axis('off')
    fig = plt.gcf()
